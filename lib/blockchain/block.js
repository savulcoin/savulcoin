...
class Block {
    toHash(useCache=false) {
        ...
        if (this._hashCache !== null) {
            return CryptoUtil.hash(this._hashCache + this.nonce);
        }

        this._hashCache = this.index + this.previousHash + this.timestamp + JSON.stringify(this.transactions);
        return this.toHash();
    }
